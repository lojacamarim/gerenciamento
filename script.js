...
// ============================================
// GOOGLE DRIVE COMPLETE SYSTEM - BACKUP & RESTORE
// ============================================
...
class DriveManager {
    ...
    async init() {
        ...
        try {
            ...
            this.initialized = true;
            console.log('✅ DriveManager inicializado');
            return true;
        } catch (error) {
            console.error('❌ Erro ao inicializar DriveManager:', error);
            return false;
        }
    }
    ...
    async authenticate() {
        try {
            ...
            const expiry = new Date().getTime() + (authResponse.expires_in * 1000);
            localStorage.setItem('camarim-google-token', googleAccessToken);
            localStorage.setItem('camarim-google-token-expiry', expiry.toString());
            
            // Obter ou criar pasta da aplicação
    ...
    async logout() {
        try {
            ...
            localStorage.removeItem('camarim-google-token');
            localStorage.removeItem('camarim-google-token-expiry');
    ...
    async getOrCreateAppFolder() {
        try {
            // Tentar encontrar a pasta existente
            const response = await gapi.client.drive.files.list({
                q: `name='${APP_FOLDER_NAME}' and mimeType='${APP_FOLDER_MIME}' and trashed=false`,
            ...
            if (response.result.files && response.result.files.length > 0) {
                ...
                console.log(`✅ Pasta encontrada: ${this.appFolderId}`);
            } else {
                // Criar nova pasta
                const fileMetadata = {
                ...
                console.log(`✅ Pasta criada: ${this.appFolderId}`);
            }
    ...
    async uploadBackup(data, fileName = null) {
        ...
        if (!fileName) {
            ...
            fileName = `backup-camarim-${date}.json`;
        }
        ...
        try {
            ...
            const metadata = {
                ...
                description: `Backup Camarim Boutique - ${new Date().toLocaleString('pt-BR')}`
            };
            ...
            console.log('✅ Backup salvo no Drive:', result);
    ...
    async listBackups() {
        ...
        try {
            const response = await gapi.client.drive.files.list({
                q: `'${this.appFolderId}' in parents and mimeType='application/json' and trashed=false and name contains 'backup-camarim'`,
                fields: 'files(id, name, createdTime, modifiedTime, size)',
                orderBy: 'modifiedTime desc',
                spaces: 'drive'
            });

            googleBackups = response.result.files || [];
            console.log(`✅ ${googleBackups.length} backups encontrados`);
            
            return googleBackups;
        } catch (error) {
    ...
    async downloadBackup(fileId) {
        try {
            ...
            if (!downloadResponse.ok) {
                throw new Error(`Download falhou: ${downloadResponse.status}`);
            }

            const textData = await downloadResponse.text();
            let jsonData;
    ...
    async deleteBackup(fileId) {
        try {
            ...
            console.log(`✅ Backup ${fileId} deletado`);
            return true;
        } catch (error) {
        ...
        try {
            ...
            if (quota) {
                ...
                return {
                    used: usedGB,
                    limit: limitGB,
                    usagePercentage: ((quota.usage / quota.limit) * 100).toFixed(1)
                };
            }
...
// ============================================
// FUNÇÕES DE INTERFACE DO GOOGLE DRIVE
// ============================================

async function initGoogleDriveSystem() {
    try {
        driveManager = new DriveManager();
        
        console.log('✅ Sistema Google Drive inicializado');
        
        // Verificar autenticação existente
        await checkExistingAuth();
        
    } catch (error) {
        console.error('❌ Erro ao inicializar sistema Google Drive:', error);
    }
}

async function checkExistingAuth() {
    try {
        const savedToken = localStorage.getItem('camarim-google-token');
        const savedExpiry = localStorage.getItem('camarim-google-token-expiry');
        ...
        if (savedToken && savedExpiry) {
            ...
            if (now < parseInt(savedExpiry)) {
                ...
                // Inicializar gapi com token existente
                try {
                    if (typeof gapi !== 'undefined') {
                        gapi.client.setToken({ access_token: savedToken });
                        await driveManager.init();
                        await driveManager.getOrCreateAppFolder();
                        
                        updateDriveUI();
                        console.log('✅ Reconectado ao Google Drive');
                        return true;
                    }
                } catch (error) {
...
async function handleGoogleAuth() {
    try {
        if (!isGoogleAuthenticated) {
            ...
            if (typeof gapi === 'undefined') {
                showAlert('Carregando Google API...', 'info');
                // Aguardar um momento para a API carregar
                await new Promise(resolve => setTimeout(resolve, 1000));
                
                if (typeof gapi === 'undefined') {
                    throw new Error('Google API não carregada. Verifique sua conexão.');
                }
            }
            ...
            // Carregar lista de backups automaticamente
            await loadBackupsFromDrive();
            
            // Atualizar informações de armazenamento
            await updateDriveStorageInfo();
        } else {
            if (confirm('Deseja desconectar do Google Drive?')) {
                await driveManager.logout();
                updateDriveUI();
                clearBackupsList();
        ...
        console.error('❌ Erro na autenticação:', error);
        showAlert(`Erro ao conectar com Google: ${error.message}`, 'error');
    }
}
...
async function saveBackupToDrive() {
    ...
    try {
        showAlert('Criando backup dos dados...', 'info');
        ...
        const backupData = {
            ...
            exportInfo: {
                ...
                version: '1.0',
                productsCount: systemData.products.length,
                salesCount: systemData.sales.length,
                totalInventoryValue: calculateInventoryValue().totalSellingValue,
                appName: 'Camarim Boutique'
            }
        };
        
        // Fazer upload
        showAlert('Enviando backup para o Google Drive...', 'info');
        
        const result = await driveManager.uploadBackup(backupData);
        
        showAlert(`✅ Backup salvo com sucesso! Arquivo: ${result.name}`, 'success');
        ...
        showAlert(`Erro ao salvar backup: ${error.message}`, 'error');
    }
}

async function loadBackupsFromDrive() {
    if (!isGoogleAuthenticated) {
        showAlert('Conecte-se ao Google Drive primeiro', 'error');
        return;
    }
    ...
    try {
        showAlert('Buscando backups no Google Drive...', 'info');
        
        const backups = await driveManager.listBackups();
        googleBackups = backups;
        
        displayBackupsList(backups);
        
        if (backups.length === 0) {
            showAlert('Nenhum backup encontrado no Google Drive', 'warning');
        } else {
            showAlert(`${backups.length} backup(s) encontrado(s)`, 'success');
        }
...
function displayBackupsList(backups) {
    ...
    const listContainer = document.getElementById('drive-backups-list');
    ...
    backups.forEach((backup, index) => {
        const date = new Date(backup.modifiedTime || backup.createdTime);
        ...
        let sizeText = '';
        if (backup.size) {
            const sizeKB = Math.ceil(backup.size / 1024);
            sizeText = `${sizeKB} KB`;
        }
        
        // Extrair nome amigável
        let friendlyName = backup.name;
        if (backup.name.startsWith('backup-camarim-')) {
            friendlyName = backup.name.replace('backup-camarim-', '').replace('.json', '');
            friendlyName = friendlyName.replace(/-/g, ' ');
        }
        
        html += `
            <div class="backup-item ${isLatest ? 'latest' : ''}" data-id="${backup.id}">
                <div class="backup-info">
                    <div class="backup-name">
                        <i class="fas fa-database"></i> ${DOMPurify.sanitize(friendlyName)}
                    </div>
                    <div class="backup-meta">
                        <span class="backup-date"><i class="far fa-calendar"></i> ${formattedDate}</span>
                        ${sizeText ? `<span class="backup-size"><i class="far fa-hdd"></i> ${sizeText}</span>` : ''}
                    </div>
                </div>
                <div class="backup-actions">
                    <button class="btn btn-sm btn-primary btn-backup-load" 
                            onclick="restoreFromBackup('${backup.id}', '${DOMPurify.sanitize(backup.name)}')">
                        <i class="fas fa-download"></i> Carregar
                    </button>
                    <button class="btn btn-sm btn-danger btn-backup-delete" 
                            onclick="deleteBackupFromDrive('${backup.id}', '${DOMPurify.sanitize(backup.name)}')">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
...
async function restoreFromBackup(fileId, fileName) {
    if (!confirm(`Deseja carregar o backup "${fileName}"?\n\nATENÇÃO: Todos os dados atuais serão substituídos.`)) {
        return;
    }
    
    try {
        showAlert(`Carregando backup "${fileName}"...`, 'info');
        
        const backup = await driveManager.downloadBackup(fileId);
        
        // Validar estrutura do backup
        if (!backup.data.systemData || !backup.data.systemData.products || !Array.isArray(backup.data.systemData.products)) {
            throw new Error('Estrutura de backup inválida. Dados de produtos não encontrados.');
        }
        
        // Validar versão (opcional)
        if (backup.data.exportInfo && backup.data.exportInfo.appName !== 'Camarim Boutique') {
            if (!confirm('Este backup parece ser de outro sistema. Deseja continuar mesmo assim?')) {
                return;
            }
        }
        
        // Substituir dados do sistema
        systemData = backup.data.systemData;
        
        // Garantir que os IDs estão atualizados
        updateProductIds();
        updateSaleIds();
        ...
        showAlert(`✅ Backup "${fileName}" carregado com sucesso!\nSistema recarregado com ${systemData.products.length} produtos e ${systemData.sales.length} vendas.`, 'success');
        
    } catch (error) {
        console.error('❌ Erro ao restaurar backup:', error);
        showAlert(`Erro ao carregar backup: ${error.message}`, 'error');
    }
}
...
async function deleteBackupFromDrive(fileId, fileName) {
    ...
    try {
        showAlert(`Excluindo backup "${fileName}"...`, 'info');
        
        await driveManager.deleteBackup(fileId);
        
        // Atualizar lista local
        googleBackups = googleBackups.filter(backup => backup.id !== fileId);
        displayBackupsList(googleBackups);
...
function updateDriveUI() {
    ...
    const statusDiv = document.getElementById('drive-connection-status');
    const driveSection = document.getElementById('drive-section');
    
    if (isGoogleAuthenticated) {
        if (loginBtn) {
            loginBtn.innerHTML = '<i class="fab fa-google"></i> Desconectar do Google';
        ...
        if (statusDiv) {
            statusDiv.innerHTML = '<span class="status-dot online"></span> <strong>Conectado ao Google Drive</strong>';
            statusDiv.className = 'drive-status connected';
        }
        if (driveSection) {
            driveSection.classList.add('connected');
        }
    } else {
        if (loginBtn) {
        ...
        if (statusDiv) {
            statusDiv.innerHTML = '<span class="status-dot offline"></span> <strong>Desconectado</strong>';
            statusDiv.className = 'drive-status disconnected';
        }
        if (driveSection) {
            driveSection.classList.remove('connected');
        }
...
async function updateDriveStorageInfo() {
    if (!isGoogleAuthenticated) return;
    
    try {
        const storageInfo = await driveManager.getStorageInfo();
        const storageElement = document.getElementById('drive-storage-info');
        
        if (storageElement && storageInfo) {
            storageElement.innerHTML = DOMPurify.sanitize(`
                <div class="storage-info">
                    <div class="storage-bar">
                        <div class="storage-fill" style="width: ${storageInfo.usagePercentage}%"></div>
                    </div>
                    <div class="storage-text">
                        Usado: ${storageInfo.used} GB / ${storageInfo.limit} GB (${storageInfo.usagePercentage}%)
                    </div>
                </div>
            `);
        }
...
// ============================================
// CONFIGURAÇÃO DOS EVENT LISTENERS PARA DRIVE
// ============================================

function setupDriveEventListeners() {
    // Adicionar event listeners específicos do Google Drive
    ...
    if (saveToDriveBtn) {
        saveToDriveBtn.addEventListener('click', saveBackupToDrive);
    }
    
    if (loadFromDriveBtn) {
        loadFromDriveBtn.addEventListener('click', loadBackupsFromDrive);
    }
    ...
    if (syncDriveBtn) {
        syncDriveBtn.addEventListener('click', async () => {
            ...
            try {
                showAlert('Sincronizando com Google Drive...', 'info');
                await loadBackupsFromDrive();
                await updateDriveStorageInfo();
                showAlert('Sincronização completa!', 'success');
            } catch (error) {
    ...
    if (autoBackupBtn) {
        autoBackupBtn.addEventListener('click', () => {
            const autoBackup = localStorage.getItem('camarim-auto-backup') === 'true';
            localStorage.setItem('camarim-auto-backup', (!autoBackup).toString());
            
            if (!autoBackup) {
                showAlert('Backup automático ativado! O sistema fará backup a cada alteração importante.', 'success');
            } else {
                showAlert('Backup automático desativado.', 'info');
            }
...
function updateAutoBackupUI() {
    ...
    const isActive = localStorage.getItem('camarim-auto-backup') === 'true';
    
    if (isActive) {
        autoBackupBtn.innerHTML = '<i class="fas fa-toggle-on"></i> Auto Backup: ON';
        autoBackupBtn.classList.remove('btn-outline-primary');
        autoBackupBtn.classList.add('btn-success');
    } else {
        autoBackupBtn.innerHTML = '<i class="fas fa-toggle-off"></i> Auto Backup: OFF';
        autoBackupBtn.classList.remove('btn-success');
        autoBackupBtn.classList.add('btn-outline-primary');
    }
}

// Função para backup automático após mudanças importantes
async function triggerAutoBackup() {
    if (!isGoogleAuthenticated) return;
    
    const autoBackupEnabled = localStorage.getItem('camarim-auto-backup') === 'true';
    if (!autoBackupEnabled) return;
    
    // Evitar múltiplos backups em sequência rápida
    const lastBackupTime = localStorage.getItem('camarim-last-backup-time');
    const now = Date.now();
    
    if (lastBackupTime && (now - parseInt(lastBackupTime)) < 300000) { // 5 minutos
        console.log('Backup automático: Muito recente, pulando...');
        return;
    }
    ...
    try {
        ...
        const date = new Date();
        const fileName = `backup-auto-${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}-${date.getHours()}-${date.getMinutes()}.json`;
        
        await driveManager.uploadBackup(backupData, fileName);
        
        localStorage.setItem('camarim-last-backup-time', now.toString());
...
// ============================================
// MODIFICAÇÃO DA FUNÇÃO saveData PARA BACKUP AUTOMÁTICO
// ============================================
...
async function saveData() {
    try {
        ...
        if (success) {
            ...
            // Disparar backup automático se configurado
            setTimeout(() => {
                triggerAutoBackup();
            }, 1000);
...
// ============================================
// INICIALIZAÇÃO DO SISTEMA DRIVE
// ============================================

// Modificar a função initSystem para incluir inicialização do Drive
async function initSystem() {
    console.log('🚀 Inicializando Sistema Camarim...');
    
    try {
        // Inicializar sistema principal
        await initMainSystem();
        
        // Inicializar sistema Google Drive (em segundo plano)
        setTimeout(() => {
            initGoogleDriveSystem();
        }, 1000);
        
        // Configurar event listeners do Drive
...
console.log('✅ Módulo Google Drive Backup & Restore carregado');