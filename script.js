...
const GOOGLE_CLIENT_ID = '821978818510-ia36jn3fn9ucqgl27jmtbaqeee9kujmp.apps.googleusercontent.com'; // Você precisará criar no Google Cloud Console
const GOOGLE_API_KEY = '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';
...
function startGoogleAuth() {
    // Usar Google Identity Services para login
    const client = google.accounts.oauth2.initTokenClient({
        ...
        callback: (response) => {
            if (response.error) {
                ...
                showAlert('Erro ao conectar com Google: ' + response.error, 'error');
                return;
            }
...
function updateGoogleDriveUI() {
    ...
    if (isGoogleAuthenticated) {
        ...
        if (statusDiv) {
            statusDiv.innerHTML = '<div class="alert alert-success"><i class="fas fa-check-circle"></i> Conectado ao Google Drive</div>';
        }
    } else {
        if (loginBtn) {
        ...
        if (statusDiv) {
            statusDiv.innerHTML = '<div class="alert alert-info"><i class="fas fa-info-circle"></i> Conecte-se ao Google para salvar no Drive</div>';
        }
    }
}
...
async function saveToGoogleDrive() {
    if (!isGoogleAuthenticated || !googleAccessToken) {
        showAlert('Você precisa se conectar ao Google primeiro', 'error');
        return;
    }
    
    try {
        showAlert('Preparando dados para salvar no Drive...', 'info');
        
        // 1. Obter todos os dados do sistema
        const dataToSave = {
            ...systemData,
        ...
        const fileName = `camarim-backup-${new Date().toISOString().slice(0,10)}.json`;
        
        // 4. Upload para Google Drive
        await uploadToDrive(fileName, blob);
        
        showAlert('Backup salvo no Google Drive com sucesso!', 'success');
        
    } catch (error) {
        console.error('❌ Erro ao salvar no Google Drive:', error);
        showAlert('Erro ao salvar no Google Drive: ' + error.message, 'error');
    }
}

async function uploadToDrive(fileName, fileData) {
    // Método 1: Usar Google Drive API v3
    try {
        // Criar metadados do arquivo
        const metadata = {
            name: fileName,
            mimeType: 'application/json',
            parents: [] // Pode especificar uma pasta específica se quiser
        };
        ...
        console.log('✅ Arquivo salvo no Drive:', result);
        return result;
...
async function loadFromGoogleDrive() {
    ...
    try {
        ...
        const response = await fetch('https://www.googleapis.com/drive/v3/files?q=mimeType="application/json"&orderBy=modifiedTime desc&pageSize=10', {
            headers: {
                'Authorization': 'Bearer ' + googleAccessToken
            }
        });
        ...
        if (files.files && files.files.length > 0) {
            // Mostrar modal com arquivos disponíveis
            showDriveFilesModal(files.files);
        } else {
        ...
        showAlert('Erro ao carregar do Google Drive: ' + error.message, 'error');
    }
}

function showDriveFilesModal(files) {
    const modalContent = document.getElementById('drive-files-content');
    if (!modalContent) return;
    
    let html = '<h4>Backups disponíveis no Google Drive</h4>';
    html += '<div class="drive-files-list" style="max-height: 300px; overflow-y: auto;">';
    
    files.forEach(file => {
        if (file.name.includes('camarim-backup-')) {
            const date = file.name.replace('camarim-backup-', '').replace('.json', '');
            html += `
                <div class="drive-file-item" data-id="${file.id}">
                    <div>
                        <strong>${file.name}</strong>
                        <div class="text-muted">Modificado: ${new Date(file.modifiedTime).toLocaleString('pt-BR')}</div>
                    </div>
                    <button class="btn btn-small btn-primary load-drive-file" data-id="${file.id}">
    ...
    // Adicionar event listeners
    setTimeout(() => {
        document.querySelectorAll('.load-drive-file').forEach(button => {
            button.addEventListener('click', async function() {
                const fileId = this.getAttribute('data-id');
                await downloadFromDrive(fileId);
                hideModal('drive-files-modal');
            });
        });
    }, 100);
    
    showModal('drive-files-modal');
}

async function downloadFromDrive(fileId) {
    try {
        showAlert('Carregando backup do Google Drive...', 'info');
        ...
        systemData = {
            ...
            settings: jsonData.settings || systemData.settings
        };
        ...
        showAlert('Backup carregado do Google Drive com sucesso!', 'success');
...
// ============================================
// MODAL PARA ARQUIVOS DO DRIVE (HTML necessário)
// ============================================

// Adicione este modal ao seu HTML:
/*
<div class="modal" id="drive-files-modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Google Drive</h3>
            <button class="modal-close" data-modal="drive-files-modal">&times;</button>
        </div>
        <div class="modal-body">
            <div id="drive-files-content"></div>
        </div>
        <div class="modal-footer">
            <button class="btn btn-secondary" data-modal="drive-files-modal">Cancelar</button>
        </div>
    </div>
</div>
*/
...
// ============================================
// ATUALIZAR FUNÇÃO setupEventListeners
// ============================================

// Na função setupEventListeners(), adicione:
function setupEventListeners() {
    // ... seu código existente ...
    
    // Botão para carregar do Drive
...
// ============================================
// ATUALIZAR FUNÇÃO showView
// ============================================

// Na função showView(), adicione:
switch(viewName) {
    // ... casos existentes ...
    case 'database':
        updateDatabaseInfo();
        updateGoogleDriveUI(); // Atualizar status do Google Drive
        break;
}
