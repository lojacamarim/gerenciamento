...
function checkHashAndShowView() {
    ...
    if (hash && validViews.includes(hash)) {
        ...
        // ============================================
    // EVENT LISTENERS PARA GITHUB GIST
    // ============================================
    ...
    // Botão de ajuda/configuração
    const setupGuideBtn = document.getElementById('setup-guide-btn');
    if (setupGuideBtn) {
        setupGuideBtn.addEventListener('click', showGitHubSetupGuide);
    }
    
    // Modal de configuração do GitHub
    const configGitHubBtn = document.getElementById('config-github-btn');
    if (configGitHubBtn) {
        configGitHubBtn.addEventListener('click', configureGitHubToken);
    }
    
    // Inicializar configurações do Gist
    setTimeout(() => {
        setupGitHubGist();
    }, 1000);
}
...
// ============================================
// 21. INTEGRAÇÃO COM GITHUB GIST (NUVEM)
// ============================================

function setupGitHubGist() {
    // Botão no menu principal
    const cloudBtn = document.getElementById('cloud-btn');
    if (cloudBtn) {
        cloudBtn.addEventListener('click', () => showModal('cloud-modal'));
    }
    
    // Botões no modal de nuvem
    const saveToGistBtn = document.getElementById('save-to-gist');
    const loadFromGistBtn = document.getElementById('load-from-gist');
    const configureGistBtn = document.getElementById('configure-gist');
    const clearGistConfigBtn = document.getElementById('clear-gist-config');
    const autoSyncToggle = document.getElementById('auto-sync-toggle');
    const syncNowBtn = document.getElementById('sync-now-btn');
    
    if (saveToGistBtn) saveToGistBtn.addEventListener('click', saveToGitHubGist);
    if (loadFromGistBtn) loadFromGistBtn.addEventListener('click', loadFromGitHubGist);
    if (configureGistBtn) configureGistBtn.addEventListener('click', configureGitHubToken);
    if (clearGistConfigBtn) clearGistConfigBtn.addEventListener('click', clearGitHubConfig);
    if (autoSyncToggle) autoSyncToggle.addEventListener('change', toggleAutoSync);
    if (syncNowBtn) syncNowBtn.addEventListener('click', syncWithGitHubGist);
    
    // Verificar configuração salva
    updateGistStatus();
    
    // Configurar auto-sync se ativado
    if (localStorage.getItem('gist_auto_sync') === 'true') {
        setupAutoSync();
    }
}

async function configureGitHubToken() {
    showModal('github-config-modal');
    ...
    document.getElementById('save-github-token').addEventListener('click', async () => {
        ...
        if (!token) {
            showAlert('Digite um token do GitHub', 'error');
            return;
        }
        
        // Validar formato do token (começa com ghp_ ou gh)
        if (!token.startsWith('ghp_') && !token.startsWith('gho_') && !token.startsWith('ghu_') && !token.startsWith('ghs_') && !token.startsWith('ghr_')) {
            if (!confirm('Este token não parece estar no formato padrão. Tem certeza que é um token pessoal do GitHub?')) {
                return;
            }
        }
        ...
        // Testar conexão
        showAlert('Testando conexão com GitHub...', 'info');
        ...
        try {
            ...
            if (testResponse.ok) {
                ...
                showAlert(`✅ Conectado como: ${userData.login || 'Usuário GitHub'}`, 'success');
                
                // Fechar modal após sucesso
                setTimeout(() => {
                    hideModal('github-config-modal');
                    updateGistStatus();
                }, 2000);
            } else {
...
async function saveToGitHubGist() {
    try {
        ...
        if (!token) {
            if (confirm('Você precisa configurar um token do GitHub primeiro. Deseja configurar agora?')) {
                configureGitHubToken();
            }
            return;
        }
        
        showAlert('Salvando no GitHub Gist...', 'info');
        
        const dataStr = JSON.stringify(systemData, null, 2);
        const fileName = 'camarim-backup.json';
        const description = `Backup Camarim - ${new Date().toLocaleString('pt-BR')}`;
        ...
        if (response.ok) {
            // Salvar ID do Gist
            if (!existingGistId && result.id) {
                localStorage.setItem('github_gist_id', result.id);
            }
            
            // Salvar URL para compartilhamento
            if (result.html_url) {
                localStorage.setItem('github_gist_url', result.html_url);
                
                // Mostrar link para compartilhar
                showAlert(`
                    ✅ Backup salvo no GitHub Gist!<br><br>
                    <strong>Link para compartilhar:</strong><br>
                    <a href="${result.html_url}" target="_blank" style="color: white; text-decoration: underline;">
                        ${result.html_url}
                    </a><br><br>
                    <small>Copie este link e envie para seu colega</small>
                `, 'success');
                ...
                showAlert('✅ Backup salvo, mas não foi possível obter o link', 'success');
            }
        } else {
            throw new Error(result.message || 'Erro ao salvar no GitHub');
        }
        
    } catch (error) {
        console.error('❌ Erro ao salvar no GitHub Gist:', error);
        
        if (error.message.includes('401') || error.message.includes('Bad credentials')) {
            showAlert('Token inválido ou expirado. Configure um novo token.', 'error');
            setTimeout(() => {
                configureGitHubToken();
            }, 2000);
        } else if (error.message.includes('404')) {
            showAlert('Gist não encontrado. Criando um novo...', 'info');
...
async function loadFromGitHubGist() {
    try {
        ...
        if (!token) {
            showAlert('Configure um token do GitHub primeiro', 'error');
            configureGitHubToken();
            return;
        }
        
        if (!existingGistId) {
            const gistUrl = prompt('Cole a URL do Gist compartilhado (ou deixe em branco para usar o seu):');
            
            if (!gistUrl) {
                showAlert('É necessário um Gist para carregar os dados', 'error');
                return;
            }
            
            // Extrair ID do Gist da URL
            const gistIdMatch = gistUrl.match(/gist\.github\.com\/(?:[\w-]+\/)?([a-f0-9]+)/i);
            if (gistIdMatch && gistIdMatch[1]) {
                localStorage.setItem('github_gist_id', gistIdMatch[1]);
            } else {
                throw new Error('URL do Gist inválida');
            }
        }
        
        showAlert('Carregando dados do GitHub Gist...', 'info');
        ...
        if (response.ok) {
            const file = gist.files['camarim-backup.json'] || Object.values(gist.files)[0];
            
            if (!file || !file.content) {
                throw new Error('Nenhum arquivo de backup encontrado no Gist');
            }
            ...
            if (!confirm(`ATENÇÃO: Esta ação irá substituir TODOS os dados atuais.\n\nEncontrado: ${importedData.products.length} produtos e ${importedData.sales?.length || 0} vendas\n\nDeseja continuar?`)) {
                return;
            }
            ...
            systemData = {
                ...
                settings: importedData.settings || systemData.settings
            };
            ...
            showAlert(`
                ✅ Dados carregados com sucesso!<br><br>
                <strong>Resumo:</strong><br>
                • ${systemData.products.length} produtos<br>
                • ${systemData.sales.length} vendas<br>
                • Última atualização: ${new Date(gist.updated_at).toLocaleString('pt-BR')}
            `, 'success');
            
            // Salvar URL do Gist
            if (gist.html_url) {
                localStorage.setItem('github_gist_url', gist.html_url);
                updateGistStatus();
            }
            
        } else {
            throw new Error(gist.message || 'Erro ao carregar do GitHub');
        }
        
    } catch (error) {
        console.error('❌ Erro ao carregar do GitHub Gist:', error);
        
        if (error.message.includes('404')) {
            showAlert('Gist não encontrado. Verifique a URL ou ID.', 'error');
...
async function syncWithGitHubGist() {
    try {
        const token = localStorage.getItem('github_token');
        const gistId = localStorage.getItem('github_gist_id');
        
        if (!token || !gistId) {
            showAlert('Configure o GitHub Gist primeiro', 'error');
            configureGitHubToken();
            return;
        }
        
        showAlert('Sincronizando com GitHub Gist...', 'info');
        ...
        const remoteGist = await response.json();
        
        if (!response.ok) {
            throw new Error(remoteGist.message || 'Erro ao buscar dados remotos');
        }
        
        const remoteFile = remoteGist.files['camarim-backup.json'] || Object.values(remoteGist.files)[0];
        
        if (!remoteFile || !remoteFile.content) {
            throw new Error('Nenhum arquivo de backup encontrado no Gist remoto');
        }
        
        const remoteData = JSON.parse(remoteFile.content);
        const remoteTime = new Date(remoteGist.updated_at).getTime();
        const localTime = parseInt(localStorage.getItem('last_backup_time') || '0');
        
        // 2. Decidir qual conjunto de dados é mais recente
        if (remoteTime > localTime) {
            // Dados remotos são mais recentes
            if (confirm(`Dados remotos mais recentes (${new Date(remoteTime).toLocaleString('pt-BR')}).\nDeseja carregar os dados do servidor?`)) {
                systemData = remoteData;
                await saveData();
                loadData();
                updateDashboard();
                showAlert('✅ Dados sincronizados (remoto → local)', 'success');
            }
        } else if (localTime > remoteTime) {
            // Dados locais são mais recentes
            if (confirm(`Dados locais mais recentes (${new Date(localTime).toLocaleString('pt-BR')}).\nDeseja enviar para o servidor?`)) {
                await saveToGitHubGist();
            }
        } else {
            showAlert('✅ Ambos os lados estão sincronizados', 'success');
        }
...
function updateGistStatus() {
    ...
    const gistLinkElement = document.getElementById('gist-link');
    
    const token = localStorage.getItem('github_token');
    const gistId = localStorage.getItem('github_gist_id');
    const gistUrl = localStorage.getItem('github_gist_url');
    const lastBackup = localStorage.getItem('last_backup_time');
    
    if (statusElement) {
        if (token && gistId) {
            statusElement.innerHTML = '<span class="positive"><i class="fas fa-check-circle"></i> Configurado</span>';
        } else if (token) {
            statusElement.innerHTML = '<span class="warning"><i class="fas fa-exclamation-circle"></i> Token configurado</span>';
        } else {
            statusElement.innerHTML = '<span class="negative"><i class="fas fa-times-circle"></i> Não configurado</span>';
        }
    }
    
    if (lastSyncElement && lastBackup) {
        lastSyncElement.textContent = new Date(lastBackup).toLocaleString('pt-BR');
    } else if (lastSyncElement) {
        lastSyncElement.textContent = 'Nunca';
    }
    
    if (gistLinkElement && gistUrl) {
        gistLinkElement.innerHTML = `<a href="${gistUrl}" target="_blank">${gistUrl}</a>`;
        gistLinkElement.style.display = 'block';
    } else if (gistLinkElement) {
        gistLinkElement.style.display = 'none';
    }
}

function clearGitHubConfig() {
    if (confirm('Tem certeza que deseja limpar todas as configurações do GitHub?')) {
        localStorage.removeItem('github_token');
        localStorage.removeItem('github_gist_id');
        localStorage.removeItem('github_gist_url');
        localStorage.removeItem('last_backup_time');
        localStorage.removeItem('gist_auto_sync');
        
        updateGistStatus();
        showAlert('Configurações do GitHub limpas', 'success');
    }
}
...
function toggleAutoSync() {
    ...
    if (autoSyncToggle.checked) {
        localStorage.setItem('gist_auto_sync', 'true');
        
        const frequency = parseInt(syncFrequency.value) || 30;
        localStorage.setItem('sync_frequency', frequency.toString());
        
        setupAutoSync();
        showAlert(`Auto-sync ativado (a cada ${frequency} minutos)`, 'success');
    } else {
        localStorage.removeItem('gist_auto_sync');
        showAlert('Auto-sync desativado', 'info');
    }
}

function setupAutoSync() {
    const frequency = parseInt(localStorage.getItem('sync_frequency') || '30');
    ...
    // Configurar novo intervalo (em minutos)
    window.autoSyncInterval = setInterval(async () => {
        if (localStorage.getItem('gist_auto_sync') === 'true') {
            console.log('🔄 Auto-sync executando...');
            await syncWithGitHubGist();
        }
    }, frequency * 60 * 1000); // Converter minutos para milissegundos
}

// ============================================
// 22. INSTALAÇÃO E CONFIGURAÇÃO INICIAL
// ============================================

function showGitHubSetupGuide() {
    const guideHTML = `
        <div class="setup-guide">
            <h3><i class="fab fa-github"></i> Configuração do GitHub Gist</h3>
            
            <div class="guide-step">
                <h4>Passo 1: Criar Token no GitHub</h4>
                <ol>
                    <li>Acesse: <a href="https://github.com/settings/tokens/new" target="_blank">https://github.com/settings/tokens/new</a></li>
                    <li>Faça login na sua conta GitHub</li>
                    <li>Em "Note", digite: "Camarim Backup"</li>
                    <li>Marque apenas a opção <strong>"gist"</strong></li>
                    <li>Clique em "Generate token"</li>
                    <li><strong>COPIE O TOKEN</strong> (você só verá ele uma vez!)</li>
                </ol>
            </div>
            
            <div class="guide-step">
                <h4>Passo 2: Configurar no Sistema</h4>
                <ol>
                    <li>Cole o token no campo abaixo</li>
                    <li>Se já tiver um Gist, cole o ID também (opcional)</li>
                    <li>Clique em "Salvar Configuração"</li>
                </ol>
            </div>
            
            <div class="guide-step">
                <h4>Passo 3: Compartilhar</h4>
                <ol>
                    <li>Após salvar o primeiro backup, você receberá um link</li>
                    <li>Compartilhe este link com seu colega</li>
                    <li>Seu colega deve usar o mesmo link para carregar os dados</li>
                </ol>
            </div>
            
            <div class="alert alert-info">
                <i class="fas fa-lightbulb"></i>
                <strong>Dica:</strong> O Gist é privado, apenas pessoas com o link podem acessar.
            </div>
        </div>
    `;
    
    const modalContent = document.getElementById('github-guide-content');
    if (modalContent) {
        modalContent.innerHTML = DOMPurify.sanitize(guideHTML);
        showModal('github-guide-modal');
    }
}
