...
async function initGoogleDriveSystem() {
    try {
        ...
        document.getElementById('save-to-drive-btn')?.addEventListener('click', saveBackupToDrive);
        document.getElementById('load-from-drive-btn')?.addEventListener('click', loadBackupsFromDrive);
    ...
    try {
        ...
        if (savedToken && savedExpiry) {
            ...
            if (now < parseInt(savedExpiry)) {
                ...
                updateDriveUI();
                showAlert('Reconectado ao Google Drive com sucesso!', 'success');
                return true;
            } else {
                // Token expirado
                localStorage.removeItem('camarim-google-token');
        ...
        html += `
            <div class="backup-item ${isLatest ? 'latest' : ''}" data-id="${backup.id}">
                ...
                <div class="backup-actions">
                    ${sizeText ? `<span class="backup-size">${sizeText}</span>` : ''}
                    <button class="btn btn-primary btn-backup-load" onclick="restoreFromBackup('${backup.id}', '${backup.name}')">
...
async function restoreFromBackup(fileId, fileName) {
    ...
    try {
        ...
        if (!backup.data.systemData || !backup.data.systemData.products) {
        ...
        showAlert(`✅ Backup "${fileName}" carregado com sucesso!\nSistema recarregado com os dados do backup.`, 'success');
...
function updateDriveUI() {
    ...
    if (isGoogleAuthenticated) {
        ...
        if (statusDiv) {
            statusDiv.innerHTML = '<span class="status-dot online"></span> Conectado ao Google Drive';
        }
    } else {
        if (loginBtn) {
        ...
        if (statusDiv) {
            statusDiv.innerHTML = '<span class="status-dot offline"></span> Desconectado';
        }
...
async function initSystem() {
    ...
    // Inicializar sistema Google Drive
    await initGoogleDriveSystem();
}
